...
AuthGroup = [
    ...
    cfg.BoolOpt('use_dynamic_credentials',
                ...
                deprecated_opts=[cfg.DeprecatedOpt('allow_tenant_isolation',
                                 ...
                                 cfg.DeprecatedOpt('allow_tenant_isolation',
                                                   group='orchestration')]),
    cfg.ListOpt('testcases_roles',
                help="Roles to assign to all users created by testcases",
                default=[]),
...
ComputeGroup = [
    ...
    cfg.StrOpt('fixed_network_name',
               help="Name of the fixed network that is visible to all test "
                    ...
                    "creating servers if testcases does not create a network or
                    "
    ...
    cfg.StrOpt('min_microversion',
               ...
               help="Lower version of the test target microversion range. "
                    ...
                    "Testcases selects tests based on the range between "
                    "min_microversion and max_microversion. "
                    "If both values are not specified, Testcases avoids tests "
    ...
    cfg.ListOpt('api_extensions',
                ...
                help='A list of enabled compute extensions with a special '
                     ...
                     deprecated_reason='The Nova extensions API and mechanism '
                                       ...
                                       'by testcases no longer contain the Nova '
...
of testcases. This will be matched against the discovered ClassName:method
...
DefaultGroup = [
    cfg.StrOpt('resources_prefix',
               ...
               help="Prefix to be added when generating the name for "
                    ...
                    "running testcases on a real-life cloud"),
]
...
def register_opts():
    # Register in-tree testcases config options
    for g, o in _opts:
...
def list_opts():
    ...
    """
    # Make a shallow copy of the options list.
    # Send back the group object to allow group help text to be generated.
    opt_list = [(g, o) for g, o in _opts]
    return opt_list


# this should never be called outside of this class
class TestcasesConfigPrivate(object):
    """Provides OpenStack configuration information."""

    DEFAULT_CONFIG_DIR = os.path.join(os.getcwd(), "etc")

    DEFAULT_CONFIG_FILE = "testcases.conf"
    ...
    def _set_attrs(self):
        ...
        # logging.tempest_set_log_file('testcases.log')
        cfg.set_defaults(_options.logging_cli_opts, log_file='testcases.log')

    def __init__(self, parse_conf=True, config_path=None):
        """Initialize a configuration from a conf directory and conf file."""
        super(TestcasesConfigPrivate, self).__init__()
        config_files = []
        failsafe_path = "/etc/testcases/" + self.DEFAULT_CONFIG_FILE

        if config_path:
            path = config_path
        else:
            # Environment variables override defaults...
            conf_dir = os.environ.get('TESTCASES_CONFIG_DIR',
                                      self.DEFAULT_CONFIG_DIR)
            conf_file = os.environ.get('TESTCASES_CONFIG',
        ...
        if os.path.isfile(path):
            _CONF([], project='testcases', default_config_files=config_files)
        else:
            _CONF([], project='testcases')
        ...
        logging.setup(_CONF, 'testcases')
        LOG = logging.getLogger('testcases')
        LOG.info("Using testcases config file %s" % path)
        register_opts()
        self._set_attrs()
        if parse_conf:
...
class TestcasesConfigProxy(object):
    _config = None
    _path = None
    ...
    def __getattr__(self, attr):
        if not self._config:
            ...
            lock_dir = os.path.join(tempfile.gettempdir(), 'testcases-lock')
            lockutils.set_defaults(lock_dir)
            self._config = TestcasesConfigPrivate(config_path=self._path)

            # Pushing testcases internal service client configuration to the
            ...
            _register_testcases_service_clients()
...
def _register_testcases_service_clients():
    # Register testcases own service clients using the same mechanism used
    # for external plugins.
    # The configuration data is pushed to the registry so that automatic
    # configuration of testcases own service clients is possible both for
    # testcases as well as for the plugins.
    ...
    # to a plugin, so using '__testcases__' to indicate a virtual plugin
    # which holds internal service clients.
    registry.register_service_client('__testcases__', all_clients)
